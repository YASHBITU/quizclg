...
type Screen = 'landing' | 'info' | 'quiz' | 'result' | 'leaderboard' | 'blocked';
...
export default function App() {
  ...
  const [studentData, setStudentData] = useState<StudentData>({ fullName: '', rollNumber: '' });
  ...
  const getPerformanceMessage = (pct: number) => {
    ...
    if (pct >= 60) return { text: "Well Done!", color: "text-orange-400" };
  ...
  const fetchLeaderboard = async () => {
    ...
    try {
      const response = await fetch(`${GOOGLE_SHEETS_WEBHOOK_URL}?action=fetch`);
  ...
  const handleInfoSubmit = async (e: FormEvent) => {
    e.preventDefault();
    if (!studentData.fullName || !studentData.rollNumber) return;
    ...
    setIsChecking(true);
    try {
      const response = await fetch(`${GOOGLE_SHEETS_WEBHOOK_URL}?action=check&rollNumber=${studentData.rollNumber}`);
      const data = await response.json();
      if (data.exists) {
      ...
      // Fallback to quiz if check fails (optional, could also block)
      setCurrentScreen('quiz');
    } finally {
      setIsChecking(false);
    }
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-4 md:p-8 font-sans overflow-x-hidden">
          ...
          >
            <div className="space-y-6">
              ...
              <h1 className="text-6xl md:text-8xl font-display font-extrabold tracking-tight">
                Elite <span className="gradient-text">Quiz Pro</span>
              </h1>
          ...
          >
            ...
            <form onSubmit={handleInfoSubmit} className="space-y-6">
              ...
              <motion.button
                ...
                disabled={isChecking}
                type="submit"
                className="w-full py-4 bg-gradient-to-r from-purple-600 to-orange-600 text-white rounded-2xl font-bold text-lg shadow-lg shadow-purple-500/20 flex items-center justify-center gap-2"
              >
          ...
          >
            ...
            <div className="space-y-4">
              ...
              <p className="text-slate-400 leading-relaxed">
                Roll Number <span className="text-white font-bold">{studentData.rollNumber}</span> has already completed this quiz. 
                ...
                {QUIZ_QUESTIONS[currentQuestionIndex].options.map((option, idx) => {
                  ...
                  return (
                    <motion.button
                      ...
                      className={`w-full p-5 rounded-2xl text-left border transition-all flex items-center justify-between group relative overflow-hidden ${
                        isSelected 
              ...
              <div className="space-y-4">
                {submitStatus === 'idle' ? (
                  <button
                    ...
                    className="w-full py-4 bg-white text-navy-900 rounded-2xl font-bold text-lg flex items-center justify-center gap-2"
                  >
                    {isSubmitting ? <Loader2 className="w-5 h-5 animate-spin" /> : 'Save Result to Database'}
                  </button>
                ) : (
                  <div className={`p-4 rounded-2xl flex items-center gap-3 ${submitStatus === 'success' ? 'bg-emerald-500/10 text-emerald-400' : 'bg-rose-500/10 text-rose-400'}`}>
                    {submitStatus === 'success' ? <CheckCircle2 className="w-6 h-6" /> : <AlertCircle className="w-6 h-6" />}
                    <p className="text-sm font-medium">{submitStatus === 'success' ? 'Saved to Google Sheets!' : 'Submission failed.'}</p>
                  </div>
                )}
              </div>
            </div>
              ...
              <div className="flex justify-center">
                ...
                >
                  ...
                  <div className="space-y-2">
                    ...
                    <h1 className="text-5xl font-display font-black">Elite Quiz Pro</h1>
                  </div>
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="text-3xl font-bold text-orange-400">{score} / {totalQuestions} ({percentage}%)</div>
                  </div>
          ...
          >
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 px-2">
              <div className="space-y-2">
                ...
                <p className="text-slate-400">Live rankings of the top performers</p>
              </div>
                      ...
                      leaderboard.map((entry, idx) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="px-8 py-6 font-bold text-slate-200">{entry.name}</td>
                            <td className="px-8 py-6 font-mono text-sm text-slate-400">{entry.rollNumber}</td>
                            <td className="px-8 py-6">
                            ...
                            <td className="px-8 py-6">
                              <span className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest border ${
                                entry.badge === 'Gold' ? 'border-yellow-500/30 text-yellow-500 bg-yellow-500/10' :
                                entry.badge === 'Silver' ? 'border-slate-400/30 text-slate-400 bg-slate-400/10' :
                                entry.badge === 'Bronze' ? 'border-orange-500/30 text-orange-500 bg-orange-500/10' :
      ...
      <footer className="fixed bottom-4 text-[10px] text-slate-600 uppercase tracking-[0.2em] font-bold pointer-events-none">
        Elite Assessment Platform © 2026
      </footer>
    </div>
  );
}
